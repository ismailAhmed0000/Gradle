...
def test_ink_pixels_are_opaque_and_background_is_transparent():
    ...
    # text was drawn around (40-200, 90-140); corners should stay background (fully transparent)
    assert alpha[10:30, 10:30].max() == 0
    assert alpha[170:190, 370:390].max() == 0
    # somewhere in the text region should have been picked up as ink
...
def _page_with_ruled_line() -> bytes:
    image = np.full((200, 400, 3), 245, dtype=np.uint8)
    cv2.putText(image, "hello", (40, 120), cv2.FONT_HERSHEY_SIMPLEX, 2, (20, 20, 20), 3, cv2.LINE_AA)
    cv2.line(image, (0, 170), (400, 170), (30, 30, 30), 1)  # ruled line, well clear of the text
    ok, encoded = cv2.imencode(".jpg", image)
    assert ok
    return encoded.tobytes()
...
def test_ruled_line_is_removed_but_text_survives():
    ...
    # ruled line's row, away from the line's own antialiasing, should be gone
...
def test_long_wide_stroke_is_not_mistaken_for_a_ruled_line():
    # a single stroke spanning most of the crop width -- wide like a ruled line,
    # but tall like real handwriting, so it must survive (regression: an earlier
    # width-only heuristic misclassified long cursive text as a ruled line).
    image = np.full((200, 400, 3), 245, dtype=np.uint8)
...
def test_isolated_specks_are_removed_but_punctuation_survives():
    # specks like grid-line-intersection antialiasing are tiny and isolated;
    # a real punctuation mark (e.g. a period) is a similar shape but bigger.
    image = np.full((200, 400, 3), 245, dtype=np.uint8)
