...
# ruled-line suppression: after bridging small dot/dash gaps, a connected blob
# counts as a "ruled line" only if it's both wide (spans most of the crop) AND
# thin (short vertical extent). Width alone isn't enough -- a long cursive
# sentence can also form one unbroken horizontal run and must not be caught.
...
# isolated specks (e.g. anti-aliasing at grid-line intersections) land in a
# consistent small-area band well below any real stroke fragment -- even a
# single punctuation mark is comfortably larger than this.
...
def extract_ink_png(image_bytes: bytes, crop_x: float, crop_y: float, crop_width: float, crop_height: float) -> bytes:
    ...
    bgra[:, :, 3] = mask  # ink pixels black (BGR=0), alpha = mask
...
def _ink_mask(cropped: np.ndarray) -> np.ndarray:
    ...
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel, iterations=2)  # bridge small stroke gaps
...
def _ruled_line_mask(mask: np.ndarray) -> np.ndarray:
    """Isolates printed/stitched ruled lines so they can be subtracted before the
    ink mask is finalized. A component is treated as a ruled line only if it is
    both wide (spans most of the crop) and thin (small vertical extent) -- real
    handwriting, even long or cursive, occupies far more vertical space than a
    ruled line's stroke thickness."""
